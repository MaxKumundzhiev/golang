...
func main () {
	...
	- выполняются все defer в текущей функции
	- стек начинает подниматься вверх
	- если recover нигде не вызван — программа завершается

	Как работает recover()
		recover() проверяет:
			есть ли сейчас активный panic, который раскручивает стек.
			если panic есть → возвращает значение, переданное в panic(...)
			если panic нет → возвращает nil
	*/
